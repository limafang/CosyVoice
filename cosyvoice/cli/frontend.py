...
class CosyVoiceFrontEnd:
    ...
    def frontend_voice_convert(self, prompt_speech_16k, source_speech_16k):
        ...
        fragment_num = math.ceil(
            source_speech_16k.size(1) / max_speech_samples
        )  # 按29秒切分，因为speech tokenizer最大支持30s
        rhythm_speech_token = []
        rhythm_speech_token_len = 0
        for i in range(fragment_num):
            speech_16k = source_speech_16k[
                :, i * max_speech_samples : (i + 1) * max_speech_samples
            ]
